        ...
        <div class="col-span-12 mt-4">
            ...
            <div v-if="media.length" class="grid grid-cols-12 gap-4">
                <div 
                    v-for="m in media"
                    :key="m.id"
                    class="col-span-6 md:col-span-4 p-2 border rounded bg-gray-50 cursor-pointer"
                    @click="m.type === 'foto' && openFullImage(m.url)"
                >
                    <img v-if="m.type==='foto'" :src="m.url" class="w-full h-32 object-cover rounded" />
                    <video v-if="m.type==='video'" :src="m.url" controls class="w-full h-32 rounded"></video>
