...
const formattedDate = today.toLocaleDateString('es-PE').replace(/\//g, '-');
const startDownload = async (type: 'pdf' | 'excel') => {
  const url = type === 'pdf' 
    ? '/panel/reports/export-pdf-accessories' 
    : '/panel/reports/export-excel-accessories'

const filename = type === 'pdf'
  ? `Lista de Accesorios ${formattedDate}.pdf`
  : `Lista de Accesorios ${formattedDate}.xlsx`
