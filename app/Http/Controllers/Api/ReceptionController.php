...
class ReceptionController extends Controller
{
            ...
            $query->where(function ($q) use ($search) {
                ...
                $q->whereHas('owner', function ($q2) use ($search) {
                    ...
                    ->orWhereHas('contact', function ($q3) use ($search) {
                     ...
                     // Por engine
                    ->orWhereHas('engine', function ($q3) use ($search) {
                        $q3->whereRaw('LOWER(marca) LIKE ?', ["%{$search}%"])
    ...
    public function exportExcel()
    {
        ...
        $fileName = "Lista de Recepciones {$fecha}.xlsx";

       return Excel::download(new ReceptionsExport(
    request('search'),
    request('state'),
